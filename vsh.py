...
class VectorStoreHandler:
    ...
    async def initialize_doc_vectorstore(self):
        ...
        if vectorstore is None:  # Create a new vectorstore if loading failed or doesn't exist
            try:
        ...
        if vectorstore:  # Set the retriever if vectorstore was successfully loaded or created
    ...
    async def fetch_single_document(self, session: aiohttp.ClientSession, url: str) -> List[Document]:
        logging.info(f"VectorStoreHandler.fetch_single_document called with url: {url}")
        try:
    ...
    async def load_urls_from_file(self, filename='urls.txt') -> List[str]:
        logging.info(f"VectorStoreHandler.load_urls_from_file called with filename: {filename}")
        try:
            async with aiofiles.open(filename, 'r') as file:
                ...
                self.ingested_urls.update(urls)
                return urls
    ...
    async def save_ingested_urls(self, filename="ingested_urls.txt"):
        logging.info(f"VectorStoreHandler.save_ingested_urls called with filename: {filename}")
        try:
    ...
    async def add_urls_to_vectorstore(self, new_urls_str: str) -> str:
        logging.info("VectorStoreHandler.add_urls_to_vectorstore called")
    ...
    async def monitor_urls_file(self, interval=60):
        logging.info("VectorStoreHandler.monitor_urls_file called")
        while True:
            try:
    ...
    async def ensure_stores_initialized(self):
        logging.info("VectorStoreHandler.ensure_stores_initialized called")
        if not self.are_stores_initialized():
            await self.initialize_stores()

        try:
            await asyncio.wait_for(self._stores_initialized.wait(), timeout=60.0)
        except asyncio.TimeoutError:
            logger.error("Vector store initialization timed out.")
            raise

    def are_stores_initialized(self):
        logging.info("VectorStoreHandler.are_stores_initialized called")
    ...
    async def add_code_to_vectorstore(self, code: str, success: bool, results=None):
        logging.info("VectorStoreHandler.add_code_to_vectorstore called")
        await self.ensure_stores_initialized()
    ...
    def format_results(self, results) -> str:
        logging.info("VectorStoreHandler.format_results called")
        formatted = []
