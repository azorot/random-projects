...
#|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
#--------------------------------------------------------------
fine_structure_constant_window = 1/137.035999
def add_fine_structure_constant(column = 'close', window = fine_structure_constant_window ):
    df['fsc']= df[column].rolling(window=window).apply(
        lambda x: fsc(x.dropna().values)
    return df
