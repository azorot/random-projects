...
class GradioInterface:
    ...
    async def initialize_rl_agent(self, ollama_model, state_dim, action_dim, learning_rate, batch_number):  # new method to initialize the rl_agent
        logging.info(f"Initializing RL Agent with model: {ollama_model}, state_dim: {state_dim}, action_dim: {action_dim}, learning_rate: {learning_rate}, batch_number: {batch_number}")
    ...
    async def process_input(self, question, chat_history):
        logging.info(f"process_input called with question: {question}")
    ...
    def test_strategy(self):
        logging.info("test_strategy called")
    ...
    async def train_rl_agent(self, num_batches, episodes_per_batch, batch_number):  # Modified to accept num_batches and episodes_per_batch
        logging.info(f"train_rl_agent called with num_batches: {num_batches}, episodes_per_batch: {episodes_per_batch}, batch_number: {batch_number}")
        if self.rl_agent:
    ...
    def show_plots(self):
        logging.info("show_plots called")
    ...
    def create_interface(self):
        with gr.Blocks() as demo:
            ...
            episodes_per_batch_input = gr.Number(value=2, label="Episodes Per Batch", precision=0)  # New input for episodes_per_batch... # Get the value of batch_number_input *outside* the lambda
            ...
            def sync_process_input(q, history):
                logging.info(f"Button 'Submit' clicked! Calling sync_process_input with question: {q}")
            ...
            def sync_add_urls(new_urls_text):
                logging.info(f"Button 'Add URLs to Vector Store' clicked! Calling sync_add_urls with URLs: {new_urls_text}")
                loop = asyncio.get_event_loop()
                return loop.run_until_complete(self.vector_store.add_urls_to_vectorstore(new_urls_text))

            def sync_load_urls():
                logging.info("Button 'Load URLs from File' clicked! Calling sync_load_urls")
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                return loop.run_until_complete(self.vector_store.load_urls_from_file())

            def sync_test_strategy():
                logging.info("Button 'Test Strategy' clicked! Calling sync_test_strategy")
                return self.test_strategy()  # Trigger strategy test

            def sync_train_rl_agent(num_batches, episodes_per_batch, batch_number):
                logging.info(f"Button 'Train RL Agent' clicked! Calling sync_train_rl_agent with num_batches: {num_batches}, episodes_per_batch: {episodes_per_batch}, batch_number: {batch_number}")
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
            ...
            def sync_show_plots():
                logging.info("Button 'View Plots' clicked! Calling sync_show_plots")
